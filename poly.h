/** @file
  ...
  @authors Jakub Pawlewicz <pan@mimuw.edu.pl>, Marcin Peczarski <marpe@mimuw.edu.pl>, Julia Podrażka
...
/** To jest makro sprawdzające, czy poprawnie udało się zaalokować pamięć. */
#define CHECK_PTR(p)  \
  do {                \
    if (p == NULL) {  \
      exit(1);        \
...
static inline Mono MonoFromPoly(const Poly *p, poly_exp_t n) {
    if(n != 0 && PolyIsZero(p)) return (Mono) {.p = *p, .exp = 0};
 ...
 */
static inline bool PolyIsZero(const Poly *p) {
    if (!PolyIsCoeff(p) && p -> size == 1 && PolyIsZero(&p -> arr[0].p))
        return true;
...
/**
 * Sprawdza, czy wielomian @p p jest tożsamościowo równy wielomianowi stałemu
 * @param[in] p : wielomian @f$p@f$
 * @return true, jeśli wielomian tożsamościowo równy wielomianowi stałemu
 */
bool AllExpZero(const Poly *p);

/**
 * Zwraca wspołczynnik stały wielomianu @p p
 * @param[in] p : wielomian @f$p@f$, którego size równy jest jeden
 * @return współczynnik stały wielomianu @p p
 */
poly_coeff_t GetCoeff(const Poly *p);

/**
 * Składa wielomian @p p z wielomianami z tablicy @p q w taki sposób, że pod
 * zmienną @f$x_i@f$ podstawia wielomian @f$q[i]@f$. Jeśli zabraknie
 * wielomianów w tablicy @p q, to podstawia wielomian zerowy.
 * @param[in] p : wielomian @f$p@f$
 ...
 * Formalnie dla wielomianu @f$p(x_0, x_1, x_2, ...)@f$ wynikiem jest
 * wielomian @f$p(x, x_0, x_1, ...)@f$.
 * @param[in] p : wielomian @f$p@f$
 * @param[in] x : wartość argumentu @f$x@f$-
 * @return @f$p(x, x_0, x_1, ...)@f$
 */
