...
static bool SimpleAddTest(void) {
    ...
    res &= TestAdd(P(C(36), 17),
                   P(C(-1), 4, C(9), 2),
                   P(C(9), 2, C(-1), 4, C(36), 17));
    res &= TestAdd(C(1),
                   C(2),
                   C(3));
    res &= TestAdd(P(C(1), 1),
...
static bool SimpleAtTest(void) {
    bool res = true;
    res &= TestAt(P(P(C(1), 4), 0), 2, P(C(1), 4));
...
/*int main() {
    ...
    PolyDestroy(&p);
    Mono *monos1 = calloc(2, sizeof (Mono));
    assert(monos1);
    monos1[0] = M(P(C(-1), 1), 1);
    monos1[1] = M(P(C(1), 1), 2);
    Poly p1 = PolyCloneMonos(2, monos1);
    Poly p2 = PolyCloneMonos(2, monos1);
    PolyDestroy(&p1);
    PolyDestroy(&p2);
    MonoDestroy(monos1 + 0);
    MonoDestroy(monos1 + 1);
    free(monos1);
